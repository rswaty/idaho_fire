...
percent_agricultural_total <- round((sum(conversion_summary$reclass_acres[conversion_summary$reclass_evt == "Agricultural"]) / total_roi_acres) * 100)

percent_developed_total <- round((sum(conversion_summary$reclass_acres[conversion_summary$reclass_evt == "Developed"]) / total_roi_acres) * 100)
...
!["Pixel Painting" by Leo Barch after Robin Holmes (https://watercyclecolors.com/?page_id=1246)](assets/pixel_painting.png){style="float:right; margin-left: 5%; fig-align:right; width:45%;" fig-alt="\"Map of historical fire regimes."}
...
# Wrap and reverse order of top 10 BpS names
ordered_names <- rev(str_wrap(top_10_names$short_name, width = 25))

# Prepare the data
panel_arrow_long <- panel_arrow_long |>
  mutate(
    fire_type = str_to_title(as.character(fire_type)),  # e.g., "replacement" → "Replacement"
    fire_type = factor(fire_type, levels = c("Surface", "Mixed", "Replacement")),
    bps_name_wrapped = str_wrap(bps_name, width = 25),
    bps_name_wrapped = factor(bps_name_wrapped, levels = ordered_names)
  )

# Create the plot
change_three_panel_plot <-
  ggplot(panel_arrow_long, aes(y = bps_name_wrapped)) +
  geom_segment(aes(x = past, xend = current, yend = bps_name_wrapped),
               arrow = arrow(length = unit(0.25, "cm")),
               color = "#373b38", size = 1) +
  # geom_point(aes(x = past), color = "#373b38", size = 2) +
  # geom_point(aes(x = current), color = "#373b38", size = 2) +
  theme_minimal(base_size = 14) +
  theme(
    plot.caption = element_text(hjust = 0, face = "italic"),
    plot.title.position = "plot",
  ...
  facet_wrap(~ fire_type,
             scales = "free_x",
             labeller = labeller(fire_type = toupper)) +
  guides(fill = "none") +
  scale_x_continuous(labels = comma, breaks = pretty_breaks(n = 3)) +
  labs(
    title = "Change in annual acres burned due to land conversion",
    subtitle = "Split out by fire type.  Note: X-axis varies between panels.",
    x = "Annual Acres",
    y = "",
    caption = "Arrow: Past → Current"
  )
