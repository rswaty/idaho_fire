...
```{r}
#| label: wrangle input data for hist fire
#| message: false
#| warning: false
#| include: false
...
# wrangle to present data and following charts by BPS_NAME
hist_fire <- raw_data |>
  group_by(bps_name) |>
  summarise(
...
top_bps_name <- hist_fire |>
  slice_max(total_bps_acres, n = 1) |>
  pull(bps_name)
...
From a quick assessment of the historical Mean Fire Return Intervals for `r ROI` we found:

* Roughly **`r scales::comma(total_acres_burned)`** acres burned annually or **`r percent_of_roi_burned`%** of `r ROI`.
...
* Mean Fire Return Intervals for all fire types combined ranged from **`r min_all_fri `** to **`r max_all_fri `** years, with an area weighted mean of **`r area_weighted_mean_all`** (weighted by acres of BpSs)




```{r}
#| label: wrangle data for strip chart
#| message: false
#| warning: false
#| include: false
...
p <- layout(
  p,
  title = list(
    text = "Snapshot of historical fire regimes:<br>Yellows = frequent fire, purples = infrequent. Rectangle size = ecosystem size.<br> Hover over rectangle for more information.",
  ...
  xaxis = list(
    title = list(
      text = "Mean Fire Return Interval (years)",
      font = list(size = 16, color = "black")
    ),
    tickvals = c(min(strip_chart_df$xmid), max(strip_chart_df$xmid)),
    ticktext = c(
      paste0(round(min(strip_chart_df$mean_all_fri), 1), " yrs"),
      paste0(round(max(strip_chart_df$mean_all_fri), 1), " yrs")
    ),
    showgrid = FALSE,
    zeroline = FALSE
  ),
...
## Fire Type by Biophysical Setting

LANDFIRE's [Biophysical Settings](https://www.landfire.gov/vegetation/bps) tells many stories.  Officially it is:
...
For the chart and data table below we added some additional calculations such as annual acres burned by fire severity.  
...
```{r}
#| label: wrangle data for panel chart
#| message: false
#| warning: false
#| include: false
